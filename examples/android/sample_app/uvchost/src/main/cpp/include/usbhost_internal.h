...
struct USB_INTERFACE {
    ...
    int extraLength;
    int numEndpoints;
    uint8_t usbInterfaceNumber;
};

struct USB_INTERFACES {
    USB_INTERFACE interfaces[MAX_USB_INTERFACES];
    int numInterfaces;
};
...
int uvc_get_device_descriptor(usb_device *devh, uvc_device_descriptor_t **desc);
uvc_error_t uvc_query_stream_ctrl(usb_device *devh, uvc_stream_ctrl_t *ctrl, uint8_t probe, int req);

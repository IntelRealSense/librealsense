...
struct gui {
    ...
    bool slider(int id, const rect& r, double min, double max, double step, double& value)
    {
        ...
        if (mouse_down && clicked_id == id) {
            ...
            if (step)
                new_value = std::round((new_value - min) / step) * step + min;
...
int main(int argc, char* argv[]) try {
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (!dev->is_streaming()) {
            for (int i = 0; i < 4; ++i) {
                ...
                if (g.checkbox({ w - 260, y, w - 240, y + 20 }, enable)) {
                    if (enable)
                        dev->enable_stream(s, rs::preset::best_quality);
                    else
        ...
        if (g.slider(100, { w - 260, y + 16, w - 20, y + 36 }, 0, 5, 1, dc_preset))
            apply_depth_control_preset(dev, static_cast<int>(dc_preset));
        y += 38;
        g.label({ w - 260, y + 12 }, { 1, 1, 1 }, "IVCAM options preset: %g", iv_preset);
        if (g.slider(101, { w - 260, y + 16, w - 20, y + 36 }, 0, 8, 1, iv_preset))
            apply_ivcam_preset(dev, static_cast<int>(iv_preset));
        y += 38;

...
int main(int argc, char* argv[]) try {
    ...
    if (dev.is_stream_enabled(rs::stream::infrared2))
        app_state.tex_streams.push_back(rs::stream::infrared2);
    ...
    glfwSetMouseButtonCallback(win, [](GLFWwindow* win, int button, int action, int mods) {
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT)
            s->ml = action == GLFW_PRESS;
        if (button == GLFW_MOUSE_BUTTON_RIGHT && action == GLFW_PRESS)
            s->index = (s->index + 1) % s->tex_streams.size();
    });
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        std::ostringstream ss;
        ss << dev.get_name() << " (" << app_state.tex_streams[app_state.index] << ")";

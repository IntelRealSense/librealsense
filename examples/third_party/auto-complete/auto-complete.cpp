...
void AutoComplete::RegisterOnKeyCallback(const vector<char>& keys, Callback callback)
{
    ...
    if (_callback)
        throw logic_error("Already registered to on-key callback.");
...
void AutoComplete::UnregisterOnKeyCallback()
{
    ...
    if (!_callback)
        throw logic_error("There is no any registration to on-key callback.");
...
void AutoComplete::Stop()
{
    ...
    if (!_action)
        throw logic_error("Not started yet.");
...
void AutoComplete::HandleSpecialKey(vector<uint8_t> chars)
{
...
char AutoComplete::getch_nolock()
{
#if defined(WIN32)
...
void AutoComplete::Start()
{
    ...
    if (_action)
        throw logic_error("Already started.");

    _action = true;
    _thread = unique_ptr<thread>(new thread([&]()
    {
        while (_action)
        {
            ...
            if (ch == '\t')
            {
                if (_findsVec.empty())
                {
                    lock_guard<recursive_mutex> lck(_dictionaryMtx);

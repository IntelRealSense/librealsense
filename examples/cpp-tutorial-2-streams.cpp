...
int main() try
{
    ...
    while(!glfwWindowShouldClose(win))
    {
        ...
        glDrawPixels(640, 480, GL_RED, GL_UNSIGNED_SHORT, frames[rs::stream::depth].get_data());
        glPixelTransferf(GL_RED_SCALE, 1.0f);

        // Display color image as RGB triples
        glRasterPos2f(0, 1);
        glDrawPixels(640, 480, GL_RGB, GL_UNSIGNED_BYTE, frames[rs::stream::color].get_data());

        // Display infrared image by mapping IR intensity to visible luminance
        glRasterPos2f(-1, 0);
        glDrawPixels(640, 480, GL_LUMINANCE, GL_UNSIGNED_BYTE, frames[rs::stream::infrared].get_data());

        // Display second infrared image by mapping IR intensity to visible luminance
        if(dev->is_stream_enabled(rs::stream::infrared2))
        {
            glRasterPos2f(0, 0);
            glDrawPixels(640, 480, GL_LUMINANCE, GL_UNSIGNED_BYTE, frames[rs::stream::infrared2].get_data());
        }

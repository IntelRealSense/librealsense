...
#include <realdds/topics/ros2/ListParametersPubSubTypes.h>
...
list_parameters_response_msg::create_topic( std::shared_ptr< dds_participant > const & participant, char const * topic_name )
{
    return std::make_shared< dds_topic >( participant,
                                          eprosima::fastdds::dds::TypeSupport( new list_parameters_response_msg::type ),
                                          topic_name );
}


/*static*/ bool
list_parameters_response_msg::take_next( dds_topic_reader & reader, list_parameters_response_msg * output, dds_sample * sample )
{
    list_parameters_response_msg output_;
    if( ! output )
    ...
    DDS_API_CALL_THROW( "list_parameters_response_msg::take_next", status );
}

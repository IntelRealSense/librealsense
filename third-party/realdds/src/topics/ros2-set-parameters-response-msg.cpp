...
#include <realdds/topics/ros2/SetParameters.h>
#include <realdds/topics/ros2/SetParametersPubSubTypes.h>
...
set_parameters_response_msg::create_topic( std::shared_ptr< dds_participant > const & participant, char const * topic_name )
{
    return std::make_shared< dds_topic >( participant,
                                          eprosima::fastdds::dds::TypeSupport( new set_parameters_response_msg::type ),
                                          topic_name );
}


/*static*/ bool
set_parameters_response_msg::take_next( dds_topic_reader & reader, set_parameters_response_msg * output, dds_sample * sample )
{
    set_parameters_response_msg output_;
    if( ! output )
    ...
    DDS_API_CALL_THROW( "set_parameters_response_msg::take_next", status );
}


dds_sequence_number set_parameters_response_msg::write_to( dds_topic_writer & writer )
{

...
get_parameters_response_msg::create_topic( std::shared_ptr< dds_participant > const & participant, char const * topic_name )
{
    return std::make_shared< dds_topic >( participant,
                                          eprosima::fastdds::dds::TypeSupport( new get_parameters_response_msg::type ),
                                          topic_name );
}
...
bool get_parameters_response_msg::take_next( dds_topic_reader & reader, dds_sample * sample )
{
    ...
    DDS_API_CALL_THROW( "get_parameters_response_msg::take_next", status );
}

...
{
    topics::notification notification( json{
        { "id", "device-header" },
        { "n-streams", n_streams },
    } );
                                        ...
                                        dds_notification_server & notifications )
{
    ...
    for( auto & sp : stream->profiles() )
        profiles.push_back( nlohmann::ordered_json( std::move( sp->to_json() ) ) );
    json msg = {
        { "id", "stream-header" },
    ...
    topics::notification notification( msg );
    notifications.add_discovery_notification( std::move( notification ) );
}
...
void dds_device_server::publish_notification( topics::notification && notification )
{

...
args.add_argument( '--ros-discovery', action='store_true', help='listen to ros_discovery_info' )
def domain_arg(x):
    t = int(x)
    if t < 0 or t > 232:
        raise ArgumentError( f'--domain "{x}" should be [0-232]' )
    return t
...
if not args.flexible and not args.flexible_be and not args.blob and not args.image and not args.ros_discovery:
    e( '--flexible, --flexible-be, --blob, --image, or --ros2-discovery required' )
    sys.exit( 1 )
...
# ROS2 `ros_discovery_info`
if args.ros_discovery:
    def on_discovery_info_available( reader ):
        if not ready:
            return
        got_something = False
        while True:
            sample = dds.message.sample_info()
            msg = dds.message.participant_entities_info.take_next( reader, sample )
            if not msg:
                if not got_something:
                    raise RuntimeError( "expected message not received!" )
                break
            i( f'{timestamp()} {msg}', )
            got_something = True
    topic_path = dds.topics.ros2.discovery_info
    reader = dds.topic_reader( dds.message.participant_entities_info.create_topic( participant, topic_path ))
    reader.on_data_available( on_discovery_info_available )
    rqos = dds.topic_reader.qos( dds.reliability.reliable, dds.durability.transient_local )
    rqos.history.kind = dds.history_kind.keep_all  # this is what the ROS code does
    reader.run( rqos )
    i( f'Waiting for {topic_path}' )
    if args.wait and not reader.wait_for_writers( dds.time( args.wait ) ):
        e( f'Timeout waiting for writers on {topic_path}' )

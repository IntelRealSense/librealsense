...
TEST_CASE( "rs_stream_is_enabled() validates input", "[offline] [validation]" )
{
    REQUIRE(rs_stream_is_enabled(nullptr,               RS_STREAM_DEPTH,    require_error("null pointer passed for argument \"device\"")) == 0);
                                                                            
    REQUIRE(rs_stream_is_enabled(fake_object_pointer(), (rs_stream)-1,      require_error("bad enum value for argument \"stream\"")) == 0);
    REQUIRE(rs_stream_is_enabled(fake_object_pointer(), RS_STREAM_COUNT,    require_error("bad enum value for argument \"stream\"")) == 0);
    REQUIRE(rs_stream_is_enabled(fake_object_pointer(), RS_STREAM_MAX_ENUM, require_error("bad enum value for argument \"stream\"")) == 0);
}
...
TEST_CASE( "rs_device_is_streaming() validates input", "[offline] [validation]" )
{
    REQUIRE(rs_device_is_streaming(nullptr, require_error("null pointer passed for argument \"device\"")) == 0);
}

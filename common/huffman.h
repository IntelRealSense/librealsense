...
bool unhuffimage4(uint32_t *compressedImage, uint32_t lengthOfCompressedImageInU32s, uint32_t widthInBytes, uint32_t height, unsigned char *image)
{
    memcpy(((char *)(image)), ((char *)(compressedImage)), widthInBytes);
    uint32_t wordCount = (widthInBytes + 3) >> 2;
    uint32_t *currentCompressedWordP = compressedImage + wordCount;
    unsigned char *previousImageLine = image;
    unsigned char *currentImageLine = image + widthInBytes;
    uint32_t currentState = 0;
    unsigned char byteAbove = *previousImageLine++;
    uint32_t *compressedLim = compressedImage + lengthOfCompressedImageInU32s;
    ...
    unsigned char *end = image + widthInBytes * height - 32;

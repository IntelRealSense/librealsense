...
void init_device(py::module &m) {
    ...
    auto_calibrated_device.def(py::init<rs2::device>(), "device"_a)
        ...
        { 
            float health;
            return std::make_tuple(self.run_on_chip_calibration(json_content, &health, timeout_ms), health);
        ...
        {
            float health;
            return std::make_tuple(self.run_on_chip_calibration(json_content, &health, f, timeout_ms), health);
        ...
        {
            float health[2];
            return std::make_tuple(self.run_tare_calibration(ground_truth_mm, json_content, health, timeout_ms), health);
        ...
        {
            float health[2];
            return std::make_tuple(self.run_tare_calibration(ground_truth_mm, json_content, health, callback, timeout_ms), health);
        ...
        .def("add_calibration_frame", [](const rs2::auto_calibrated_device& self, rs2::frame frame, int timeout_ms)
            {
                float health[2];
                return std::make_tuple(self.add_calibration_frame(frame, health, timeout_ms), health);
            }, "This will add a frame to the calibration process initiated by run_tare_calibration or run_on_chip_calibration as host assistant process. This call is executed on the caller's thread.", "frame"_a, "timeout_ms"_a, py::call_guard<py::gil_scoped_release>())

...
var
  ...
  dev_count, NumOfFrames, I, FrameNumber, j, Y, X, numero: integer;
  dev: pRS2_device;
...
begin
  ...
  repeat
    ...
    begin
      ...
      writeln('');
      Writeln('Width: ' + IntToStr(rs2_get_frame_width(frame, e)));
      Writeln('Height: ' + IntToStr(rs2_get_frame_height(frame, e)));
      Writeln ('Size: '+intToStr (rs2_get_frame_data_size (frame,e)));
      Writeln ('Bits por pixel: '+intTostr (rs2_get_frame_bits_per_pixel(frame,e)))  ;
      ...
      scr16 := pint12(rs2_get_frame_data(frame,e));
     ...
     // fpColor.Alpha := $FFFF;
      ...
      jp := TFPWriterJPEG.Create;
      jp.CompressionQuality := 100;
      image.SaveToFile('test.jpg', jp);
      FreeAndNil(image);
      end;

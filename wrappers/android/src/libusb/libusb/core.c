...
{
	usbi_dbg("usbi_alloc_device with session_id :%d", session_id);
...
{
	usbi_dbg("usbi_disconnect_device");
	struct libusb_context *ctx = DEVICE_CTX(dev);
	...
	libusb_device ***list)
{
	...
	if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
		usbi_dbg("backend provides hotplug support");
		/* backend provides hotplug support */
		struct libusb_device *dev;

#ifndef ANDROID //weiguo
		if (usbi_backend.hotplug_poll) //op_hotplug_poll
			usbi_backend.hotplug_poll();
#endif

		usbi_dbg("try usb device");
		usbi_mutex_lock(&ctx->usb_devs_lock);
		list_for_each_entry(dev, &ctx->usb_devs, list, struct libusb_device) {
			usbi_dbg("found new usb device:%d:%d", dev->bus_number, dev->device_address);
	...
	else 
	{
		...
		usbi_dbg("backend does not provides hotplug support");
...
int API_EXPORTED libusb_init(libusb_context **context)
{
	...
	if (usbi_backend.init) {
		usbi_dbg("calling usbi_backend.init(...)");
	    //op_init
...
static void usbi_log_str(enum libusb_log_level level, const char *str)
{
	...
	__android_log_write(priority, "irsa_libusb", str);
	...
	const char *file, const char *function, int line, const char *format, va_list args)
{
	...
	if (global_debug) {
		...
		header_len = snprintf(buf, sizeof(buf),
			"[%2d.%06d] [%08x] libusb: %s [%s, %s, %d] ",
			(int)now.tv_sec, (int)(now.tv_nsec / 1000L), usbi_get_tid(), prefix, ((pfile != NULL) ? pfile + 1 : file), function, line);
	} else {
		header_len = snprintf(buf, sizeof(buf),
	...
	const char *file, const char *function, int line, const char *format, ...)
{
	...
	usbi_log_v(ctx, level, file, function, line, format, args);
	va_end (args);
}

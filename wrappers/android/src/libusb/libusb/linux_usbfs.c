...
static int check_usb_vfs(const char *dirname)
{
	...
	dir = opendir(dirname);
	if (!dir) {
		usbi_err(NULL, "could not open %s, cause: %s", dirname, strerror(errno));
		return 0;
    }
...
static const char *find_usbfs_path(void)
{
	...
	usbi_dbg("enter find_usbfs_path");
	if (found = check_usb_vfs(path)) {
	    usbi_dbg("found %d", found);
		ret = path;
	} else {
		path = "/proc/bus/usb";
...
#if defined(USE_UDEV)
	usbi_dbg("USE_UDEV scan_devices");
	ret = linux_udev_scan_devices(ctx);
#else
	usbi_dbg("default scan_devices");
...
{
	usbi_dbg("hotplug_poll");
#if defined(USE_UDEV)
	usbi_dbg("udev_hotplug_poll");
	linux_udev_hotplug_poll();
#else
	usbi_dbg("netlink_hotplug_poll");
	...
	const char *devname, const char *attr)
{
	...
	if (fd == -1) {
		...
		usbi_err(ctx, "open %s failed errno=%d(%s)", filename, errno, strerror(errno));
		return LIBUSB_ERROR_IO;
	}
...
static int usbfs_get_active_config(struct libusb_device *dev, int fd)
{
	...
	if (r < 0) {
		...
		usbi_dbg("active_config: %d", active_config);
		if (active_config > 0) {
	...
	uint8_t devaddr, const char *sysfs_dir)
{
	...
	do {
		...
		usbi_dbg("desc len :%d", priv->descriptors_len);
	} while (priv->descriptors_len == descriptors_size);

	if (0 == priv->descriptors_len) {
		usbi_dbg("using jni to get descriptor");
		descriptors_size *= 2;
		priv->descriptors = usbi_reallocf(priv->descriptors,
						  descriptors_size);
		if (!priv->descriptors) {
		...
		priv->descriptors_len = _getDescriptor(priv->descriptors);
		usbi_dbg("desc len :%d", priv->descriptors_len);
	}
...
{
	struct dirent *entry;
	usbi_dbg("usbdev_names:%d, path: %s", usbdev_names, usbfs_path);
#ifdef ANDROID //weiguo
	//weiguo
	char *uvcName = _getUVCName();
	usbi_dbg("uvcName:%s", uvcName);

	int busnum = 1;
	uint8_t devaddr = 9;
	if (uvcName != NULL) {
		char *pStr = strrchr(uvcName, '/');
		//usbi_dbg("addr %s", pStr + 1);
		usbi_dbg("addr %d", atoi(pStr + 1));
		devaddr = atoi(pStr + 1);
	}


	if (linux_enumerate_device(ctx, busnum, (uint8_t) devaddr, "usb1")) {
		usbi_dbg("failed to enumerate dir entry %d", devaddr);
		return LIBUSB_ERROR_IO;
	}
...
{
	usbi_dbg("path:%s", SYSFS_DEVICE_PATH);
	...
	if (!devices) {
		usbi_err(ctx, "opendir devices failed errno=%d(%s)", errno, strerror(errno));
		return LIBUSB_ERROR_IO;
	}
...
static int linux_default_scan_devices (struct libusb_context *ctx)
{
	...
	if (sysfs_can_relate_devices != 0) {
		usbi_dbg("sysfs_get_device_list");
		return sysfs_get_device_list(ctx);
	}
	else 
#endif
	{
		usbi_dbg("usbfs_get_device_list");

...
static void android_irsa_IrsaMgr_native_init(JNIEnv *env) {
	...
	//enable librealsense running on non-rooted Android device
...
extern "C" int _openUVCDevice() {
    ...
    if (s_Fields.openUVCDevice != nullptr) {
        CHECK(env.get() != nullptr);
		LOGV("open");
        fd = env->CallStaticIntMethod(s_Fields.gClass, s_Fields.openUVCDevice);
		LOGD("fd: %d", fd);
		LOGV("fd: %d", fd);
	}
	return fd;
...
extern "C" int _check_usb_vfs(const char *path) {
    AutoJNIEnv env;
	int found = 0;
	LOGD("path:%s", path);
...
extern "C" int _getDescriptor(char *source) {
	...
	if (iDescriptor > 0) {
		...
		memcpy(source, mDescriptor, iDescriptor);
		return iDescriptor;
	}
    ...
    if (s_Fields.getDescriptor != nullptr) {
		...
		memcpy(source, pDescription, bufSize);

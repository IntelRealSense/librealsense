...
function rs2_create_pipeline(ctx: prs2_context; error: Prs2_error)
  : prs2_pipeline; cdecl; external REALSENSE_DLL;
{
...
procedure rs2_pipeline_stop(pipe: prs2_pipeline; error: Prs2_error); cdecl;
...
procedure rs2_delete_pipeline(pipe: prs2_pipeline); cdecl;
  external REALSENSE_DLL;
{
...
procedure rs2_delete_pipeline_profile(profile: pRS2_pipeline_profile); cdecl;
  external REALSENSE_DLL;
{
...
function rs2_pipeline_poll_for_frames(pipe: prs2_pipeline;
  out_frame: pRS2_frame; error: Prs2_error): integer; cdecl;
  external REALSENSE_DLL;
{
  Wait until a new set of frames becomes available.
  The frames set includes time-synchronized frames of each enabled stream in the pipeline.
...
function rs2_pipeline_try_wait_for_frames(pipe: prs2_pipeline;
  output_frame: pRS2_frame; timeout_ms: cardinal; error: Prs2_error): integer;
...
function rs2_pipeline_start(pipe: prs2_pipeline; error: prs2_pipeline)
  : pRS2_pipeline_profile; cdecl; external REALSENSE_DLL;
{
  Start the pipeline streaming with its default configuration.
...
function rs2_pipeline_start_with_callback(pipe: prs2_pipeline;
...
function rs2_pipeline_start_with_config_and_callback(pipe: prs2_pipeline;
  config: pRs2_config; on_frame: pRS2_frame_callback_ptr; user: pvoid;
...
function rs2_pipeline_get_active_profile(pipe: prs2_pipeline; error: Prs2_error)
  : pRS2_pipeline_profile; cdecl; external REALSENSE_DLL;
{
...
function rs2_pipeline_profile_get_device(profile: prs2_pipeline;
...
function rs2_pipeline_profile_get_streams(profile: prs2_pipeline;

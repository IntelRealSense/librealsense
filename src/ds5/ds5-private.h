...
namespace librealsense
{
    ...
    namespace ds
    {
        ...
        enum fw_cmd : uint8_t
        {
            ...
            SET_PWM_ON_OFF  = 0x79,     // set emitter on and off mode
            GET_PWM_ON_OFF  = 0x7A,     // get emitter on and off mode
            SET_START_LINE  = 0x7B,     // set 848x100 vertical start line number
            GET_START_LINE  = 0x7C      // get 848x100 vertical start line number
        };
...
#pragma pack(pop)
        ...
        rs2_intrinsics get_intrinsic_by_resolution(const std::vector<uint8_t>& raw_data, calibration_table_id table_id, uint32_t width, uint32_t height, const ds5_device* owner);

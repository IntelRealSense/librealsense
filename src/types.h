...
#define LOG(SEVERITY, ...)                                                                                                                                                                             \
    do                                                                                                                                                                                                 \
    {                                                                                                                                                                                                  \
        if(static_cast<int>(SEVERITY) >= rsimpl::minimum_log_severity)                                                                                                                                 \
        {                                                                                                                                                                                              \
            std::ostringstream ss;                                                                                                                                                                     \
            ss << __VA_ARGS__;                                                                                                                                                                         \
            rsimpl::log(SEVERITY, ss.str());                                                                                                                                                           \
        }                                                                                                                                                                                              \
    } while(false)
...
#undef RS_ENUM_HELPERS
    ...
    struct pixel_format_unpacker
    {
        ...
        {
            for(auto & o : outputs)
                if(o.first == stream) return true;
            return false;
        }
        rs_format get_format(rs_stream stream) const
        {
            for(auto & o : outputs)
                if(o.first == stream) return o.second;
    ...
    inline uint32_t pack(uint8_t c0, uint8_t c1, uint8_t c2, uint8_t c3) { return (c0 << 24) | (c1 << 16) | (c2 << 8) | c3; }
}

...
native_stream::native_stream(device_config& config, rs_stream stream)
    : config(config)
    , stream(stream)
{
    for (auto& subdevice_mode : config.info.subdevice_modes) {
        for (auto pad_crop : subdevice_mode.pad_crop_options) {
            for (auto& unpacker : subdevice_mode.pf.unpackers) {
                ...
                if (selection.provides_stream(stream))
...
void native_stream::get_mode(int mode, int* w, int* h, rs_format* f, int* fps) const
{
    ...
    if (w)
        *w = selection.get_width();
    if (h)
        *h = selection.get_height();
    if (f)
        *f = selection.get_format(stream);
    if (fps)
        *fps = selection.get_framerate(stream);
}
...
{
    if (archive && archive->is_stream_enabled(stream))
        return archive->get_mode(stream);
    if (config.requests[stream].enabled) {
        for (auto subdevice_mode : config.select_modes()) {
            if (subdevice_mode.provides_stream(stream))
...
const rsimpl::byte* rectified_stream::get_frame_data() const
{
    ...
    if (get_pose() == source.get_pose() && get_intrinsics() == source.get_intrinsics())
        return source.get_frame_data();

    if (image.empty() || number != get_frame_number()) {
        if (table.empty())
            table = compute_rectification_table(get_intrinsics(), get_extrinsics_to(source), source.get_intrinsics());
        image.resize(get_image_size(get_intrinsics().width, get_intrinsics().height, get_format()));

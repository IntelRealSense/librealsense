...
#define RSTPER 1
    ...
    while (m_doFrames_run) {
        ...
        while (offset < m_framebuf_size) {
            std::shared_ptr<uint8_t> data;
            do {
                if (!m_doFrames_run) goto out;
                data = m_queue->front();
            } while (data == 0);
            chunk_header_t* ch = (chunk_header_t*)data.get();
            ...
            case 0:
                ...
                memcpy((void*)(m_framebuf.get() + offset), (void*)(data.get() + CHUNK_HLEN), ret);
                break;
            case 1: 
                try {
                    ret = ZSTD_decompress((void*)(m_framebuf.get() + offset), CHUNK_SIZE, (void*)(data.get() + CHUNK_HLEN), ch->size - CHUNK_HLEN);
                }
                catch (...) {
                ...
                break;
            case 2: 
                ret = LZ4_decompress_safe((const char*)(data.get() + CHUNK_HLEN), (char*)(m_framebuf.get() + offset), ch->size - CHUNK_HLEN, CHUNK_SIZE);
                ret = ch->size - CHUNK_HLEN;
                break;
            case 3:
#define HEAD_LEN 41
                ...
                off = data.get() + CHUNK_HLEN; // skip the header
                ptr = off;
                ptr++;
                marker_buffer = NULL;

                // find next markers
                while (ptr - data.get() < ch->size + CHUNK_HLEN) {
                    if (*ptr == 0xFF) {
                        // marker detected
                        switch(*(ptr + 1)) {
                        case 0xD9: // EOI
        ...
        uint8_t* frame_raw = new uint8_t[m_framebuf_size];
        if (m_profile.stream_type() == RS2_STREAM_COLOR) {
            rs2::video_stream_profile vsp = m_profile.as<rs2::video_stream_profile>();
            // recreate missing headers

            u_char *p = m_framebuf.get();
            u_char *start = p;
            ...
            *p++ = 0xd9; /* SOS */
            ...
            // decompress the JPEG
            try {
                jpeg::decompress(m_framebuf.get(), total_size + (p - start), frame_raw, m_framebuf_size);
                size = m_framebuf_size;
                memset(m_framebuf.get(), 0, m_framebuf_size);
                ...
                LOG_ERROR("Cannot decompress the frame, of size " << total_size << " to the buffer of " << m_framebuf_size);
            }
        } else {
            memcpy(frame_raw, m_framebuf.get(), m_framebuf_size);
        }

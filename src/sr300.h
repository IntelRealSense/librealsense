...
namespace rsimpl2
{
    ...
    public:
        ...
        {
            if(fo.metadata_size >= uvc::uvc_header_size )
    ...
    protected:
        ...
        bool has_metadata_ts(const uvc::frame_object& fo) const
        {
            // Metadata support for a specific stream is immutable
            const bool has_md_ts = [&]{ std::lock_guard<std::recursive_mutex> lock(_mtx);
                return ((fo.metadata != nullptr) && (fo.metadata_size >= uvc::uvc_header_size) && ((byte*)fo.metadata)[0] >= uvc::uvc_header_size);
            }();

            return has_md_ts;
        }

        bool has_metadata_fc(const uvc::frame_object& fo) const
        {
            // Metadata support for a specific stream is immutable

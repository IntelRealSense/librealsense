...
namespace rsimpl
{
    ...
    iv_camera::iv_camera(std::shared_ptr<uvc::device> device, const static_device_info & info, const iv::camera_calib_params & calib) :
    ...
    void iv_camera::set_options(const rs_option options[], size_t count, const double values[])
    {
        for (size_t i = 0; i < count; ++i)
        {
            ...
            case RS_OPTION_F200_ACCURACY:             iv::set_accuracy(get_device(), static_cast<uint8_t>(values[i])); break;

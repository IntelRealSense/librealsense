...
public:
    auto_exposure_mechanism(std::shared_ptr<rsimpl::uvc_endpoint> dev, auto_exposure_state auto_exposure_state);
    ~auto_exposure_mechanism();
    void add_frame(rs_frame* frame);
...
private:
    void push_back_data(rs_frame* data);
    bool try_pop_front_data(rs_frame** data);
    ...
    std::shared_ptr<rsimpl::uvc_endpoint>  _device;

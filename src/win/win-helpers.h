...
#define LOG_HR(x) check(#x, x, false);
        ...
        bool parse_usb_path_single_interface(uint16_t & vid, uint16_t & pid, uint16_t & mi, std::string & unique_id, const std::string & path);
        ...
        bool get_device_serial(uint16_t device_vid, uint16_t device_pid, const std::string& device_uid, std::string& serial);

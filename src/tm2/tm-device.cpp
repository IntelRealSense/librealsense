                    ...
                    if(tm_sensor_type != SensorType::Fisheye ||
                        (tm_profile.wWidth == sp.width && tm_profile.wHeight == sp.height &&
                         rs2_format_from_tm2(tm_profile.bPixelFormat) == sp.format)) {
                             LOG_IF(tm_sensor_type == SensorType::Fisheye &&_device->usb_info.conn_spec < platform::usb3_type, ERROR) << "Streaming T265 video over USB " << platform::usb_spec_names.at(_device->usb_info.conn_spec) << " is unreliable and may result in frame drops. It is recommended to use this connection for transmitting Pose/IMU data only.";

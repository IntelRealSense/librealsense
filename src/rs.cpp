...
namespace rsimpl {
template <class T>
void stream_args(std::ostream& out, const char* names, const T& last) { out << names << ':' << last; }
...
static void translate_exception(const char* name, std::string args, rs_error** error)
{
    try {
        ...
        if (error)
            *error = new rs_error{ e.what(), name, move(args) };
    } // todo - Handle case where THIS code throws
    catch (...) {
        if (error)
            *error = new rs_error{ "unknown error", name, move(args) };
    } // todo - Handle case where THIS code throws
}
}
...
#define VALIDATE_NOT_NULL(ARG) \
    if (!ARG)                  \
        throw std::runtime_error("null pointer passed for argument \"" #ARG "\"");

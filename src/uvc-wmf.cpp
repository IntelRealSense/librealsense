...
namespace uvc {
    ...
    {
        if (FAILED(hr))
            throw std::runtime_error(to_string() << call << "(...) returned 0x" << std::hex << (uint32_t)hr);
    }
    ...
    public:
        reader_callback(std::weak_ptr<device> owner, int subdevice_index)
            : owner(owner)
    ...
    struct subdevice {
        ...
        com_ptr<IMFMediaSource> get_media_source()
        {
            if (!mf_media_source) {
                ...
                if (SUCCEEDED(mf_media_source->QueryInterface(__uuidof(IAMVideoProcAmp), (void**)&am_video_proc_amp)))
                    LOG_DEBUG("obtained IAMVideoProcAmp");
            }
        ...
        IKsControl* get_ks_control(const uvc::extension_unit& xu)
        {
            ...
            if (node_type != KSNODETYPE_DEV_SPECIFIC_LOCAL)
                throw std::runtime_error(to_string() << "Invalid extension unit node ID: " << xu.node);
    ...
    struct device {
        ...
        device(std::shared_ptr<context> parent, int vid, int pid, std::string unique_id)
            : parent(move(parent))
            , vid(vid)
            , pid(pid)
        ...
        void stop_streaming()
        {
            ...
            while (true) {
                ...
                for (auto& sub : subdevices)
                    is_streaming |= sub.reader_callback->is_streaming();
                if (is_streaming)
        ...
        void open_win_usb(const guid& interface_guid, int interface_number) try {
            ...
            for (int member_index = 0;; ++member_index) {
                ...
                if (detail_data->DevicePath == nullptr)
                    continue;

                // Check if this is our device
                int usb_vid, usb_pid, usb_mi;
                std::string usb_unique_id;
                if (!parse_usb_path(usb_vid, usb_pid, usb_mi, usb_unique_id, win_to_utf(detail_data->DevicePath)))
                    continue;
                if (usb_vid != vid || usb_pid != pid || usb_mi != interface_number || usb_unique_id != unique_id)
                    continue;
        ...
        {
            if (usb_interface_handle == INVALID_HANDLE_VALUE)
                throw std::runtime_error("winusb has not been initialized");
    ...
    HRESULT reader_callback::OnReadSample(HRESULT hrStatus, DWORD dwStreamIndex, DWORD dwStreamFlags, LONGLONG llTimestamp, IMFSample* sample)
    {
        if (auto owner_ptr = owner.lock()) {
            ...
            case MF_E_INVALIDREQUEST:
                LOG_ERROR("ReadSample returned MF_E_INVALIDREQUEST");
                break;
            case MF_E_INVALIDSTREAMNUMBER:
                LOG_ERROR("ReadSample returned MF_E_INVALIDSTREAMNUMBER");
                break;
            case MF_E_NOTACCEPTING:
                LOG_ERROR("ReadSample returned MF_E_NOTACCEPTING");
                break;
            case E_INVALIDARG:
                LOG_ERROR("ReadSample returned E_INVALIDARG");
                break;
            case MF_E_VIDEO_RECORDING_DEVICE_INVALIDATED:
                LOG_ERROR("ReadSample returned MF_E_VIDEO_RECORDING_DEVICE_INVALIDATED");
                break;
            default:
                LOG_ERROR("ReadSample returned HRESULT " << std::hex << (uint32_t)hr);
                break;
            }
            if (hr != S_OK)
                streaming = false;
    ...
    void set_subdevice_mode(device& device, int subdevice_index, int width, int height, uint32_t fourcc, int fps, std::function<void(const void* frame)> callback)
    {
        ...
        for (DWORD j = 0;; j++) {
            ...
            if (uvc_width != width || uvc_height != height)
                continue;

            check("IMFMediaType::GetGUID", media_type->GetGUID(MF_MT_SUBTYPE, &subtype));
            if (reinterpret_cast<const big_endian<uint32_t>&>(subtype.Data1) != fourcc)
                continue;
    ...
    void get_pu_control_range(const device& device, int subdevice, rs_option option, int* min, int* max)
    {
        ...
        if (option == RS_OPTION_COLOR_EXPOSURE) {
            ...
            if (min)
                *min = win_to_uvc_exposure(minVal);
            if (max)
                *max = win_to_uvc_exposure(maxVal);
            return;
        }
        for (auto& pu : pu_controls) {
            if (option == pu.option) {
    ...
    std::vector<std::shared_ptr<device>> query_devices(std::shared_ptr<context> context)
    {
        ...
        for (UINT32 i = 0; i < numDevices; ++i) {
            ...
            *&pDevice = ppDevices[i];
            ...
            if (!parse_usb_path(vid, pid, mi, unique_id, name))
                continue;
            ...
            if (subdevice_index >= dev->subdevices.size())
                dev->subdevices.resize(subdevice_index + 1);

...
namespace rsimpl {
    namespace zr300
    {
        ...
        void xu_read(const uvc::device & device, uvc::extension_unit xu, ds::control xu_ctrl, void * buffer, uint32_t length)
        {
            uvc::get_control_with_retry(device, xu, static_cast<int>(xu_ctrl), buffer, length);
        }

        void xu_write(uvc::device & device, uvc::extension_unit xu, ds::control xu_ctrl, void * buffer, uint32_t length)
        {

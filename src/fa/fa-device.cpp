...
namespace librealsense
{
    ...
    std::vector<uint8_t> fa_device::get_raw_calibration_table(fa::calibration_table_id table_id) const
    {
        command cmd(ds::GETINTCAL, table_id);
        return _hw_monitor->send(cmd);
    }

    std::vector<uint8_t> fa_device::get_new_calibration_table() const
    {
    ...
    ds::d400_caps fa_device::parse_device_capabilities(const uint16_t pid) const
    {
        using namespace ds;
    ...
    /*std::shared_ptr<synthetic_sensor> fa_device::create_ir_device(std::shared_ptr<context> ctx,
        const std::vector<platform::uvc_device_info>& all_device_infos)
    {
        ...
        //auto raw_ir_ep = std::make_shared<uvc_sensor>("Raw Infrared Sensor", std::make_shared<platform::multi_pins_uvc_device>(depth_devices),
        ...
        raw_ir_ep->register_xu(depth_xu); // make sure the XU is initialized every time we power the camera
    ...
    fa_device::fa_device(std::shared_ptr<context> ctx,
    ...
    {
        _ir_device_idx = 0;// add_sensor(create_ir_device(ctx, group.uvc_devices));
        init(ctx, group);
    }

    void fa_device::init(std::shared_ptr<context> ctx,
        const platform::backend_device_group& group)
    {
        using namespace fa;

        auto&& backend = ctx->get_backend();
        auto& raw_sensor = get_raw_ir_sensor();
            ...
            _hw_monitor = std::make_shared<hw_monitor>(
                std::make_shared<locked_transfer>(
                    std::make_shared<command_transfer_over_xu>(
                        raw_sensor, ir_xu, DS5_HWMONITOR),
                    raw_sensor));
        }
        ...
        environment::get_instance().get_extrinsics_graph().register_same_extrinsics(*_right_ir_stream, *_left_ir_stream);
        environment::get_instance().get_extrinsics_graph().register_extrinsics(*_right_ir_stream, *_right_ir_stream, _left_right_extrinsics);
        ...
        std::string device_name = (rs_fa_sku_names.end() != rs_fa_sku_names.find(pid)) ? rs_fa_sku_names.at(pid) : "RS4xx";
        ...
        auto& raw_ir_sensor = get_raw_ir_sensor();
        ...
        if (usb_modality)
        {
            _usb_mode = raw_ir_sensor.get_usb_specification();
        ...
        {
            ir_sensor.register_processing_block(processing_block_factory::create_id_pbf(RS2_FORMAT_Z16H, RS2_STREAM_DEPTH));
        }

        //if (advanced_mode && (_usb_mode >= usb3_type))
        if (_usb_mode >= usb3_type)
        {
            ir_sensor.register_processing_block(
                { {RS2_FORMAT_Y8I} },
        ...
        if ((pid == RS450_PID) && _fw_version >= firmware_version("5.12.0.1"))
        {
            ir_sensor.register_option(RS2_OPTION_HARDWARE_PRESET,
                std::make_shared<uvc_xu_option<uint8_t>>(raw_ir_sensor, ir_xu, DS5_HARDWARE_PRESET,
                    "Hardware pipe configuration"));
            ir_sensor.register_option(RS2_OPTION_LED_POWER,
                std::make_shared<uvc_xu_option<uint16_t>>(raw_ir_sensor, ir_xu, DS5_LED_PWR,
        ...
        if (_fw_version >= firmware_version("5.6.3.0"))
        {
            ...
            if ((group.usb_devices.size() > 0))
            {
                _hw_monitor = std::make_shared<hw_monitor>(
                    std::make_shared<locked_transfer>(
                        std::make_shared<command_transfer_over_xu>(
                            raw_ir_sensor, ir_xu, DS5_HWMONITOR),
                        raw_ir_sensor));
            }
#endif

            ir_sensor.register_pu(RS2_OPTION_GAIN);
            auto exposure_option = std::make_shared<uvc_xu_option<uint32_t>>(raw_ir_sensor,
                ir_xu,
                DS5_EXPOSURE,
                "Depth Exposure (usec)");
            ir_sensor.register_option(RS2_OPTION_EXPOSURE, exposure_option);

            auto enable_auto_exposure = std::make_shared<uvc_xu_option<uint8_t>>(raw_ir_sensor,
                ir_xu,
                DS5_ENABLE_AUTO_EXPOSURE,
                "Enable Auto Exposure");
            ir_sensor.register_option(RS2_OPTION_ENABLE_AUTO_EXPOSURE, enable_auto_exposure);

            ir_sensor.register_option(RS2_OPTION_EXPOSURE,
        ...
        {
            ir_sensor.register_option(RS2_OPTION_OUTPUT_TRIGGER_ENABLED,
                std::make_shared<uvc_xu_option<uint8_t>>(raw_ir_sensor, ir_xu, DS5_EXT_TRIGGER,
                    "Generate trigger from the camera to external device once per frame"));

            auto error_control = std::unique_ptr<uvc_xu_option<uint8_t>>(new uvc_xu_option<uint8_t>(raw_ir_sensor, ir_xu, DS5_ERROR_REPORTING, "Error reporting"));

            _polling_error_handler = std::unique_ptr<polling_error_handler>(
                new polling_error_handler(1000,
                    std::move(error_control),
                    raw_ir_sensor.get_notifications_processor(),
                    std::unique_ptr<notification_decoder>(new fa_notification_decoder())));

            ir_sensor.register_option(RS2_OPTION_ERROR_POLLING_ENABLED, std::make_shared<polling_errors_disable>(_polling_error_handler.get()));
            
            //ir_sensor.register_option(RS2_OPTION_ASIC_TEMPERATURE,
              //  std::make_shared<asic_and_projector_temperature_options>(raw_ir_sensor,
        ...
        {
            ir_sensor.register_option(RS2_OPTION_EMITTER_ON_OFF, std::make_shared<alternating_emitter_option>(*_hw_monitor, &raw_depth_sensor));
        }
        else if (_fw_version >= firmware_version("5.10.9.0") &&
            _fw_version.experimental()) // Not yet available in production firmware
        {
            ir_sensor.register_option(RS2_OPTION_EMITTER_ON_OFF, std::make_shared<emitter_on_and_off_option>(*_hw_monitor, &raw_depth_sensor));
        }

        if (_fw_version >= firmware_version("5.9.15.1"))
        {
            ir_sensor.register_option(RS2_OPTION_INTER_CAM_SYNC_MODE,
                std::make_shared<external_sync_mode>(*_hw_monitor));
        }

        roi_sensor_interface* roi_sensor = dynamic_cast<roi_sensor_interface*>(&ir_sensor);
        if (roi_sensor)
        ...
        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_TIMESTAMP, make_uvc_header_parser(&uvc_header::timestamp));
        ...
        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_COUNTER, make_attribute_parser(&md_capture_timing::frame_counter, md_capture_timing_attributes::frame_counter_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_SENSOR_TIMESTAMP, make_rs400_sensor_ts_parser(make_uvc_header_parser(&uvc_header::timestamp),
        ...
        ir_sensor.register_metadata(RS2_FRAME_METADATA_WHITE_BALANCE, make_attribute_parser(&md_capture_stats::white_balance, md_capture_stat_attributes::white_balance_attribute, md_prop_offset));
        ...
        ir_sensor.register_metadata(RS2_FRAME_METADATA_GAIN_LEVEL, make_attribute_parser(&md_depth_control::manual_gain, md_depth_control_attributes::gain_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_ACTUAL_EXPOSURE, make_attribute_parser(&md_depth_control::manual_exposure, md_depth_control_attributes::exposure_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_AUTO_EXPOSURE, make_attribute_parser(&md_depth_control::auto_exposure_mode, md_depth_control_attributes::ae_mode_attribute, md_prop_offset));

        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_LASER_POWER, make_attribute_parser(&md_depth_control::laser_power, md_depth_control_attributes::laser_pwr_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_LASER_POWER_MODE, make_attribute_parser(&md_depth_control::emitterMode, md_depth_control_attributes::emitter_mode_attribute, md_prop_offset,
        ...
        ir_sensor.register_metadata(RS2_FRAME_METADATA_EXPOSURE_PRIORITY, make_attribute_parser(&md_depth_control::exposure_priority, md_depth_control_attributes::exposure_priority_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_EXPOSURE_ROI_LEFT, make_attribute_parser(&md_depth_control::exposure_roi_left, md_depth_control_attributes::roi_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_EXPOSURE_ROI_RIGHT, make_attribute_parser(&md_depth_control::exposure_roi_right, md_depth_control_attributes::roi_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_EXPOSURE_ROI_TOP, make_attribute_parser(&md_depth_control::exposure_roi_top, md_depth_control_attributes::roi_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_EXPOSURE_ROI_BOTTOM, make_attribute_parser(&md_depth_control::exposure_roi_bottom, md_depth_control_attributes::roi_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_EMITTER_MODE, make_attribute_parser(&md_depth_control::emitterMode, md_depth_control_attributes::emitter_mode_attribute, md_prop_offset));
        ir_sensor.register_metadata(RS2_FRAME_METADATA_FRAME_LED_POWER, make_attribute_parser(&md_depth_control::ledPower, md_depth_control_attributes::led_power_attribute, md_prop_offset));
        ...
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_HW_TYPE, make_attribute_parser(&md_configuration::hw_type, md_configuration_attributes::hw_type_attribute, md_prop_offset));
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_SKU_ID, make_attribute_parser(&md_configuration::sku_id, md_configuration_attributes::sku_id_attribute, md_prop_offset));
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_FORMAT, make_attribute_parser(&md_configuration::format, md_configuration_attributes::format_attribute, md_prop_offset));
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_WIDTH, make_attribute_parser(&md_configuration::width, md_configuration_attributes::width_attribute, md_prop_offset));
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_HEIGHT, make_attribute_parser(&md_configuration::height, md_configuration_attributes::height_attribute, md_prop_offset));
        ir_sensor.register_metadata((rs2_frame_metadata_value)RS2_FRAME_METADATA_ACTUAL_FPS, std::make_shared<ds5_md_attribute_actual_fps>());

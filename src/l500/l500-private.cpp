    ...
    namespace ivcam2
    {
        ...
        unsigned char weeks_since_factory_calibration( const std::string & serial )
        {
            ...
            if( Y == 9 )  // using Y from serial number to get manufactoring year
                ...
                man_year = 2020 + Y;
            int man_ww = std::stoi(
                serial.substr( 2, 2 ) );  // using WW from serial number to get manufactoring work week
            ...
            int curr_ww = curr_time->tm_yday / 7;       // The tm_yday field contains the number of days sine 1st of January, we divide by 7 to get current work week

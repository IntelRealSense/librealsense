...
namespace librealsense
{
    ...
    float d500_device::get_stereo_baseline_mm() const // to be ds6 adapted
    {
        using namespace ds;
    ...
    std::vector<uint8_t> d500_device::get_d500_raw_calibration_table(ds::d500_calibration_table_id table_id) const // to be ds6 adapted
    {
        using namespace ds;
        command cmd(GET_HKR_CONFIG_TABLE, 
            static_cast<int>(d500_calib_location::d500_calib_eeprom), 
            static_cast<int>(table_id),
            static_cast<int>(d500_calib_type::d500_calib_dynamic));
        return _hw_monitor->send(cmd);
    }

    std::vector<uint8_t> d500_device::get_new_calibration_table() const // to be ds6 adapted
    {
        command cmd(ds::RECPARAMSGET);
        return _hw_monitor->send(cmd);
    }

    ds::d400_caps d500_device::parse_device_capabilities() const // to be ds6 adapted
    {
        using namespace ds;

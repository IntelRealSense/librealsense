...
namespace librealsense
{
    ...
    // Declaration for solving cyclic usage between frame_to_string and operator<<
    inline std::ostream& operator<<(std::ostream& out, const frame_interface& f);

    // We place this functions inside an anonymous namespace to avoid error of multiple symbols found on linker.
    namespace
    {
        std::string frame_to_string(const frame_interface& f)
        {
            ...
            else
            {
                ...
                {
                    s << "[" << rs2_stream_to_string(f.get_stream()->get_stream_type());

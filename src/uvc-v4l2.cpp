...
namespace uvc {
    ...
    struct subdevice {
        ...
        subdevice(const std::string& name)
            : dev_name("/dev/" + name)
            , vid()
            , pid()
            , fd()
        ...
        void get_control(const extension_unit& xu, uint8_t control, void* data, size_t size)
        {
            ...
            if (xioctl(fd, UVCIOC_CTRL_QUERY, &q) < 0)
                throw_error("UVCIOC_CTRL_QUERY:UVC_GET_CUR");
        }
        ...
        void set_control(const extension_unit& xu, uint8_t control, void* data, size_t size)
        {
            ...
            if (xioctl(fd, UVCIOC_CTRL_QUERY, &q) < 0)
                throw_error("UVCIOC_CTRL_QUERY:UVC_SET_CUR");
        }
        ...
        static void poll(const std::vector<subdevice*>& subdevices)
        {
            ...
            for (auto* sub : subdevices) {
                if (FD_ISSET(sub->fd, &fds)) {
                    ...
                    if (xioctl(sub->fd, VIDIOC_QBUF, &buf) < 0)
                        throw_error("VIDIOC_QBUF");
                }
            }
        }
    ...
    {
        if (!device.usb_handle)
            throw std::logic_error("called uvc::bulk_transfer before uvc::claim_interface");
    ...
    void set_pu_control(device& device, int subdevice, rs_option option, int value)
    {
        ...
        if (xioctl(device.subdevices[subdevice]->fd, VIDIOC_S_CTRL, &control) < 0)
            throw_error("VIDIOC_S_CTRL");
    }

    int get_pu_control(const device& device, int subdevice, rs_option option)
    {
        struct v4l2_control control = { get_cid(option) };
        if (xioctl(device.subdevices[subdevice]->fd, VIDIOC_G_CTRL, &control) < 0)

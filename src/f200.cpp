...
void f200_camera::set_options(const rs_option options[], int count, const double values[])
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        case RS_OPTION_F200_LASER_POWER:
            f200::set_laser_power(get_device(), static_cast<uint8_t>(values[i]));
            break;
        case RS_OPTION_F200_ACCURACY:
            f200::set_accuracy(get_device(), static_cast<uint8_t>(values[i]));
            break;
        case RS_OPTION_F200_MOTION_RANGE:
            f200::set_motion_range(get_device(), static_cast<uint8_t>(values[i]));
            break;
        case RS_OPTION_F200_FILTER_OPTION:
            f200::set_filter_option(get_device(), static_cast<uint8_t>(values[i]));
            break;
        case RS_OPTION_F200_CONFIDENCE_THRESHOLD:
            f200::set_confidence_threshold(get_device(), static_cast<uint8_t>(values[i]));
            break;
        case RS_OPTION_SR300_DYNAMIC_FPS:
            f200::set_dynamic_fps(get_device(), static_cast<uint8_t>(values[i]));
            break; // IVCAM 1.5 Only

        case RS_OPTION_SR300_WAKEUP_DEV_RESET:
            sr300::reset_wakeup_device(get_device(), usbMutex);
            break;

        case RS_OPTION_SR300_AUTO_RANGE_ENABLE_MOTION_VERSUS_RANGE:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::enableMvR, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_ENABLE_LASER:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::enableLaser, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MIN_MOTION_VERSUS_RANGE:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::minMvR, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MAX_MOTION_VERSUS_RANGE:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::maxMvR, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_START_MOTION_VERSUS_RANGE:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::startMvR, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MIN_LASER:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::minLaser, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MAX_LASER:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::maxLaser, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_START_LASER:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::startLaser, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_UPPER_THRESHOLD:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::ARUpperTh, values[i]);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_LOWER_THRESHOLD:
            arr_writer.set(&f200::IVCAMAutoRangeRequest::ARLowerTh, values[i]);
            break;

        case RS_OPTION_SR300_WAKEUP_DEV_PHASE1_PERIOD:
            arr_wakeup_dev_writer.set(&sr300::wakeup_dev_params::phase1Period, values[i]);
            break;
        case RS_OPTION_SR300_WAKEUP_DEV_PHASE1_FPS:
            arr_wakeup_dev_writer.set(&sr300::wakeup_dev_params::phase1FPS, (int)values[i]);
            break;
        case RS_OPTION_SR300_WAKEUP_DEV_PHASE2_PERIOD:
            arr_wakeup_dev_writer.set(&sr300::wakeup_dev_params::phase2Period, values[i]);
            break;
        case RS_OPTION_SR300_WAKEUP_DEV_PHASE2_FPS:
            arr_wakeup_dev_writer.set(&sr300::wakeup_dev_params::phase2FPS, (int)values[i]);
            break;
...
void f200_camera::get_options(const rs_option options[], int count, double values[])
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        case RS_OPTION_SR300_AUTO_RANGE_ENABLE_MOTION_VERSUS_RANGE:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::enableMvR);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_ENABLE_LASER:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::enableLaser);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MIN_MOTION_VERSUS_RANGE:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::minMvR);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MAX_MOTION_VERSUS_RANGE:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::maxMvR);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_START_MOTION_VERSUS_RANGE:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::startMvR);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MIN_LASER:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::minLaser);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_MAX_LASER:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::maxLaser);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_START_LASER:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::startLaser);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_UPPER_THRESHOLD:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::ARUpperTh);
            break;
        case RS_OPTION_SR300_AUTO_RANGE_LOWER_THRESHOLD:
            values[i] = arr_reader.get(&f200::IVCAMAutoRangeRequest::ARLowerTh);
            break;

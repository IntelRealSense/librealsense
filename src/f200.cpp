...
namespace rsimpl
{
    ...
    {
        return {dims.x, dims.y, (c.Kc[0][2] * 0.5f + 0.5f) * dims.x, (c.Kc[1][2] * 0.5f + 0.5f) * dims.y, c.Kc[0][0] * 0.5f * dims.x, c.Kc[1][1] * 0.5f * dims.y, RS_DISTORTION_INVERSE_BROWN_CONRADY,
            {c.Invdistc[0], c.Invdistc[1], c.Invdistc[2], c.Invdistc[3], c.Invdistc[4]}};
    }
    ...
    static const f200_mode f200_color_modes[] = {{{1920, 1080}, {2, 5, 15, 30}}, {{1280, 720}, {2, 5, 15, 30}}, {{960, 540}, {2, 5, 15, 30, 60}}, {{848, 480}, {2, 5, 15, 30, 60}},
        {{640, 480}, {2, 5, 15, 30, 60}}, {{640, 360}, {2, 5, 15, 30, 60}}, {{424, 240}, {2, 5, 15, 30, 60}}, {{320, 240}, {2, 5, 15, 30, 60}}, {{320, 180}, {2, 5, 15, 30, 60}}};
    static const f200_mode f200_depth_modes[] = {{{640, 480}, {2, 5, 15, 30, 60}}, {{640, 240}, {2, 5, 15, 30, 60, 110}}};
    static const f200_mode f200_ir_only_modes[] = {{{640, 480}, {30, 60, 120, 240, 300}}, {{640, 240}, {30, 60, 120, 240, 300}}};
    ...
    static static_device_info get_f200_info(const f200::CameraCalibrationParameters & c)
    {
        ...
        info.options = {{RS_OPTION_F200_LASER_POWER, 0.0, 16.0, 1.0}, {RS_OPTION_F200_ACCURACY, 1.0, 3.0, 1.0}, {RS_OPTION_F200_MOTION_RANGE, 0.0, 100.0, 1.0},
            {RS_OPTION_F200_FILTER_OPTION, 0.0, 7.0, 1.0}, {RS_OPTION_F200_CONFIDENCE_THRESHOLD, 0.0, 15.0, 1.0}};
    ...
    static const f200_mode sr300_color_modes[] = {{{1920, 1080}, {5, 15, 30}}, {{1280, 720}, {5, 15, 30, 60}}, {{960, 540}, {5, 15, 30, 60}}, {{848, 480}, {5, 15, 30, 60}},
        {{640, 480}, {5, 15, 30, 60}}, {{640, 360}, {5, 15, 30, 60}}, {{424, 240}, {5, 15, 30, 60}}, {{320, 240}, {5, 15, 30, 60}}, {{320, 180}, {5, 15, 30, 60}}};
    static const f200_mode sr300_depth_modes[] = {{{640, 480}, {5, 15, 30, 60}}, {{640, 240}, {5, 15, 30, 60, 110}}};
    static const f200_mode sr300_ir_only_modes[] = {{{640, 480}, {30, 60, 120, 200}}};
    ...
    static static_device_info get_sr300_info(const f200::CameraCalibrationParameters & c)
    {
        ...
        info.options = {
            {RS_OPTION_F200_LASER_POWER, 0.0, 16.0, 1.0}, {RS_OPTION_F200_ACCURACY, 1.0, 3.0, 1.0}, {RS_OPTION_F200_MOTION_RANGE, 0.0, 100.0, 1.0}, {RS_OPTION_F200_FILTER_OPTION, 0.0, 7.0, 1.0},
            {RS_OPTION_F200_CONFIDENCE_THRESHOLD, 0.0, 15.0, 1.0},

            {RS_OPTION_SR300_DYNAMIC_FPS, 0.0f, 0.0, 0.0}, // 2.0,        60.0,        1.0},
            {RS_OPTION_SR300_AUTO_RANGE_ENABLE_MOTION_VERSUS_RANGE, 0.0, 2.0, 1.0}, {RS_OPTION_SR300_AUTO_RANGE_ENABLE_LASER, 0.0, 1.0, 1.0},
            ...
            {RS_OPTION_SR300_AUTO_RANGE_START_MOTION_VERSUS_RANGE, (double)SHRT_MIN, (double)SHRT_MAX, 1.0}, {RS_OPTION_SR300_AUTO_RANGE_MIN_LASER, (double)SHRT_MIN, (double)SHRT_MAX, 1.0},
            {RS_OPTION_SR300_AUTO_RANGE_MAX_LASER, (double)SHRT_MIN, (double)SHRT_MAX, 1.0}, {RS_OPTION_SR300_AUTO_RANGE_START_LASER, (double)SHRT_MIN, (double)SHRT_MAX, 1.0},
            {RS_OPTION_SR300_AUTO_RANGE_UPPER_THRESHOLD, 0.0, (double)USHRT_MAX, 1.0}, {RS_OPTION_SR300_AUTO_RANGE_LOWER_THRESHOLD, 0.0, (double)USHRT_MAX, 1.0},
            {RS_OPTION_SR300_WAKEUP_DEV_PHASE1_PERIOD, 0.0, (double)USHRT_MAX, 1.0}, {RS_OPTION_SR300_WAKEUP_DEV_PHASE1_FPS, 0.0, ((double)sr300::e_suspend_fps::eFPS_MAX) - 1, 1.0},
            {RS_OPTION_SR300_WAKEUP_DEV_PHASE2_PERIOD, 0.0, (double)USHRT_MAX, 1.0}, {RS_OPTION_SR300_WAKEUP_DEV_PHASE2_FPS, 0.0, ((double)sr300::e_suspend_fps::eFPS_MAX) - 1, 1.0},
            {RS_OPTION_SR300_WAKEUP_DEV_RESET, 0.0, 0.0, 1.0}, {RS_OPTION_SR300_WAKE_ON_USB_REASON, 0.0, (double)sr300::wakeonusb_reason::eMaxWakeOnReason, 1.0},
    ...
    f200_camera::f200_camera(std::shared_ptr<uvc::device> device, const static_device_info & info, const f200::CameraCalibrationParameters & calib, const f200::IVCAMTemperatureData & temp,
    ...
    f200_camera::~f200_camera()
    {
        ...
        if(temperatureThread.joinable()) temperatureThread.join();
    }
    ...
    void f200_camera::temperature_control_loop()
    {
        ...
        const float tempFromHFOV = (tan(thermal_loop_params.HFOVsensitivity * (float)M_PI / 360) * (1 + base_calibration.Kc[0][0] * base_calibration.Kc[0][0])) /
                                   (FcxSlope * (1 + base_calibration.Kc[0][0] * tan(thermal_loop_params.HFOVsensitivity * (float)M_PI / 360)));
        ...
        while(runTemperatureThread)
        {
            ...
            {
                LOG_ERROR("TemperatureControlLoop: " << e.what());
            }
        }
    }
    ...
    {
        auto arr_writer = make_struct_interface<f200::IVCAMAutoRangeRequest>([this]() { return arr; },
            [this](f200::IVCAMAutoRangeRequest r) {
        ...
        auto arr_wakeup_dev_writer = make_struct_interface<sr300::wakeup_dev_params>([this]() { return arr_wakeup_dev_param; },
            [this](sr300::wakeup_dev_params param) {
        ...
        for(int i = 0; i < count; ++i)
        {
            ...
            case RS_OPTION_SR300_DYNAMIC_FPS:
                f200::set_dynamic_fps(get_device(), static_cast<uint8_t>(values[i]));
                break; // IVCAM 1.5 Only
    ...
    void f200_camera::get_options(const rs_option options[], int count, double values[])
    {
        ...
        for(int i = 0; i < count; ++i)
        {
            ...
            switch(options[i])
            {
            case RS_OPTION_F200_LASER_POWER:
                f200::get_laser_power(get_device(), val);
                values[i] = val;
                break;
            case RS_OPTION_F200_ACCURACY:
                f200::get_accuracy(get_device(), val);
                values[i] = val;
                break;
            case RS_OPTION_F200_MOTION_RANGE:
                f200::get_motion_range(get_device(), val);
                values[i] = val;
                break;
            case RS_OPTION_F200_FILTER_OPTION:
                f200::get_filter_option(get_device(), val);
                values[i] = val;
                break;
            case RS_OPTION_F200_CONFIDENCE_THRESHOLD:
                f200::get_confidence_threshold(get_device(), val);
                values[i] = val;
                break;
            case RS_OPTION_SR300_DYNAMIC_FPS:
                f200::get_dynamic_fps(get_device(), val);
                values[i] = val;
                break; // IVCAM 1.5 Only
            ...
            case RS_OPTION_SR300_WAKE_ON_USB_REASON:
                sr300::get_wakeup_reason(get_device(), usbMutex, val);
                values[i] = val;
                break;
            case RS_OPTION_SR300_WAKE_ON_USB_CONFIDENCE:
                sr300::get_wakeup_confidence(get_device(), usbMutex, val);
                values[i] = val;
                break;
    ...
    std::shared_ptr<frame_timestamp_reader> f200_camera::create_frame_timestamp_reader() const { return std::make_shared<rolling_timestamp_reader>(); }

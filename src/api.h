...
#define NOARGS_HANDLE_EXCEPTIONS_AND_RETURN_VOID() catch(...) { librealsense::translate_exception(__FUNCTION__, "", error); }
    ...
    #define VALIDATE_FIXED_SIZE(ARG, SIZE) if((ARG) != (SIZE)) { std::ostringstream ss; ss << "Unsupported FW image size provided { " << ARG << " }" " expecting { " << SIZE << " }"; throw librealsense::invalid_value_exception(ss.str()); }

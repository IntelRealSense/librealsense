...
namespace rsimpl
{
    namespace uvc
    {
        ...
        public:
            ...
            virtual std::vector<uvc_device_info> enumerate_uvc_devices() const = 0;

            virtual std::unique_ptr<usb_device> create_usb_device(uvc_device_info info) const = 0;
            virtual std::vector<usb_device_info> enumerate_usb_devices() const = 0;

            virtual ~backend() = default;
        };

        std::unique_ptr<backend> create_backend(std::string id);

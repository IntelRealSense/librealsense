...
namespace rsimpl
{
    ...
    const char * get_string(rs_option value)
    {
        ...
        CASE(FISHEYE_COLOR_AUTO_EXPOSURE)
        CASE(FISHEYE_COLOR_AUTO_EXPOSURE_MODE)
        CASE(FISHEYE_COLOR_AUTO_EXPOSURE_RATE)
        CASE(FISHEYE_COLOR_AUTO_EXPOSURE_SAMPLE_RATE)
        CASE(FISHEYE_COLOR_AUTO_EXPOSURE_SKIP_FRAMES)
        default: assert(!is_valid(value)); return unknown;
        }
        #undef CASE
    }

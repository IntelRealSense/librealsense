...
namespace rsimpl
{
    namespace f200
    {
        ...
        void xu_read(const uvc::device & device, uint8_t xu_ctrl, void * buffer, uint32_t length) { uvc::get_control_with_retry(device, depth_xu, static_cast<int>(xu_ctrl), buffer, length); }

        void xu_write(uvc::device & device, uint8_t xu_ctrl, void * buffer, uint32_t length) { uvc::set_control_with_retry(device, depth_xu, static_cast<int>(xu_ctrl), buffer, length); }

        void get_laser_power(const uvc::device & device, uint8_t & laser_power) { xu_read(device, IVCAM_DEPTH_LASER_POWER, &laser_power, sizeof(laser_power)); }

        void set_laser_power(uvc::device & device, uint8_t laser_power) { xu_write(device, IVCAM_DEPTH_LASER_POWER, &laser_power, sizeof(laser_power)); }

        void get_accuracy(const uvc::device & device, uint8_t & accuracy) { xu_read(device, IVCAM_DEPTH_ACCURACY, &accuracy, sizeof(accuracy)); }

        void set_accuracy(uvc::device & device, uint8_t accuracy) { xu_write(device, IVCAM_DEPTH_ACCURACY, &accuracy, sizeof(accuracy)); }

        void get_motion_range(const uvc::device & device, uint8_t & motion_range) { xu_read(device, IVCAM_DEPTH_MOTION_RANGE, &motion_range, sizeof(motion_range)); }

        void set_motion_range(uvc::device & device, uint8_t motion_range) { xu_write(device, IVCAM_DEPTH_MOTION_RANGE, &motion_range, sizeof(motion_range)); }

        void get_filter_option(const uvc::device & device, uint8_t & filter_option) { xu_read(device, IVCAM_DEPTH_FILTER_OPTION, &filter_option, sizeof(filter_option)); }

        void set_filter_option(uvc::device & device, uint8_t filter_option) { xu_write(device, IVCAM_DEPTH_FILTER_OPTION, &filter_option, sizeof(filter_option)); }

        void get_confidence_threshold(const uvc::device & device, uint8_t & conf_thresh) { xu_read(device, IVCAM_DEPTH_CONFIDENCE_THRESH, &conf_thresh, sizeof(conf_thresh)); }

        void set_confidence_threshold(uvc::device & device, uint8_t conf_thresh) { xu_write(device, IVCAM_DEPTH_CONFIDENCE_THRESH, &conf_thresh, sizeof(conf_thresh)); }

        void get_dynamic_fps(const uvc::device & device, uint8_t & dynamic_fps) { return xu_read(device, IVCAM_DEPTH_DYNAMIC_FPS, &dynamic_fps, sizeof(dynamic_fps)); }

        void set_dynamic_fps(uvc::device & device, uint8_t dynamic_fps) { return xu_write(device, IVCAM_DEPTH_DYNAMIC_FPS, &dynamic_fps, sizeof(dynamic_fps)); }
        ...
        int prepare_usb_command(uint8_t * request, size_t & requestSize, uint32_t op, uint32_t p1 = 0, uint32_t p2 = 0, uint32_t p3 = 0, uint32_t p4 = 0, uint8_t * data = 0, size_t dataLength = 0)
        {
            ...
            if(requestSize < IVCAM_MONITOR_HEADER_SIZE) return 0;
        ...
        void send_hw_monitor_command(uvc::device & device, std::timed_mutex & mutex, IVCAMCommandDetails & details)
        {
            ...
            if(details.receivedCommandDataLength > 0) memcpy(details.receivedCommandData, outputBuffer + 4, details.receivedCommandDataLength);
        }
        ...
        void perform_and_send_monitor_command(uvc::device & device, std::timed_mutex & mutex, IVCAMCommand & newCommand)
        {
            ...
            fill_usb_buffer(opCodeXmit, newCommand.Param1, newCommand.Param2, newCommand.Param3, newCommand.Param4, newCommand.data, newCommand.sizeOfSendCommandData, details.sendCommandData,
        ...
        void set_auto_range(uvc::device & device, std::timed_mutex & mutex, int enableMvR, int16_t minMvR, int16_t maxMvR, int16_t startMvR, int enableLaser, int16_t minLaser, int16_t maxLaser,
            int16_t startLaser, int16_t ARUpperTH, int16_t ARLowerTH)
        {
        ...
        void get_f200_calibration_raw_data(uvc::device & device, std::timed_mutex & usbMutex, uint8_t * data, size_t & bytesReturned)
        {
            ...
            if(prepare_usb_command(request, requestSize, (uint32_t)IVCAMMonitorCommand::GetCalibrationTable) <= 0) throw std::runtime_error("usb transfer to retrieve calibration data failed");
        ...
        {
            int r = 0;
            for(int i = 0; i < bufsize; i++) r = r * 10 + *buf++;
            return r;
        }
        ...
        int get_version_of_calibration(uint8_t * validation, uint8_t * version)
        {
            ...
            if(memcmp(valid, validation, 2) != 0)
                return 0;
            else
                return bcdtoint(version, 2);
        }
        ...
        std::tuple<CameraCalibrationParameters, IVCAMTemperatureData, IVCAMThermalLoopParams> get_f200_calibration(uint8_t * rawCalibData, int len)
        {
            ...
            if(ver > IVCAM_MIN_SUPPORTED_VERSION)
            {
                ...
                auto fixWithVersionInfo = [&](IVCAMCalibration & d, int size, uint8_t * data) { memcpy((uint8_t *)&d + sizeof(int), data, size - sizeof(int)); };
        ...
        void generate_asic_calibration_coefficients(const CameraCalibrationParameters & compensated_calibration, std::vector<int> resolution, const bool isZMode, float * values)
        {
            ...
            float coeffs[NUM_OF_CALIBRATION_COEFFS] = {1.0f, 3.0f, a, a1, b, c, d0, d1, d2, d3, d4, d5, q, p1, p2, p3, p4, p5, p6, p7, p8, h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, f0, f1, f2,
                f3, f4, f5, f6, f7, f8, f9, f10, f11, o1, o2, o3, o4, dp1, dp2, dp3, dp4, dp5, ip0, ip1, ip2, ip3, ip4, ip5, ypscale, ypoff, 0, 0};
    ...
    namespace sr300
    {
        void set_wakeup_device(uvc::device & device, std::timed_mutex & mutex, const uint32_t & phase1Period, const uint32_t & phase1FPS, const uint32_t & phase2Period, const uint32_t & phase2FPS)
        {
            ...
            if(!params.isValid()) throw std::logic_error("missing/invalid wake_up command parameters");
        ...
        void get_wakeup_reason(uvc::device & device, std::timed_mutex & mutex, unsigned char & cReason)
        {
            ...
            if(cmdWUReason.receivedCommandDataLength >= 4) // TODO - better guard condition ?
            {
                ...
                if(rslt >= (uint8_t)wakeonusb_reason::eMaxWakeOnReason) throw std::logic_error("undefined wakeonusb_reason provided");
                cReason = rslt;
            }
            else

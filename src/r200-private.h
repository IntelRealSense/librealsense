...
namespace rsimpl
{
    namespace r200
    {
        ...
        template <class T> T xu_read(const uvc::device & dev, control ctrl)
        {
            T val;
            xu_read(dev, ctrl, &val, sizeof(val));
            return val;
        }
        template <class T> void xu_write(uvc::device & dev, control ctrl, const T & value)
        {
            T val = value;
            xu_write(dev, ctrl, &val, sizeof(val));
        }
...
#pragma pack(push, 1)
        ...
        {
            uint32_t rate, value;
        }; // Framerate dependent value, such as exposure or gain
        struct temperature
        {
            int8_t current, min, max, min_fault;
        };
        struct discovery
        {
            uint32_t fps, min, max, default_value, resolution;
        }; // Fields other than fps are in same units as field (exposure in tenths of a millisecond, gain as percent)
#pragma pack(pop)

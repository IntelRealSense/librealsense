...
frame_archive::frame_archive(const std::vector<subdevice_mode_selection>& selection, rs_stream key_stream)
...
void frame_archive::cull_frames()
{
    ...
    for (auto s : other_streams) {
        while (true) {
            ...
            if (abs(t0 - frames[key_stream].front().timestamp) < abs(t1 - frames[key_stream].front().timestamp))
                break;
            discard_frame(s);
        }
    }
...
{
    if (!frontbuffer[stream].data.empty())
        freelist.push_back(std::move(frontbuffer[stream]));

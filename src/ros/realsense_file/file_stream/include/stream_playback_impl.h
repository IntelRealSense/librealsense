...
namespace rs
{
    namespace file_format
    {
        ...
        public:
            ...
            virtual rs::file_format::status set_filter(std::vector<std::string> topics) override;

            virtual rs::file_format::status seek_to_time(file_types::nanoseconds seek_time) override;

            virtual rs::file_format::status read_vendor_data(std::vector<std::shared_ptr<ros_data_objects::vendor_data>>& vendor_data,
                                            uint32_t device_id) const override;
            virtual rs::file_format::status read_stream_infos(std::vector<std::shared_ptr<ros_data_objects::stream_info>>& stream_infos,
                                             file_types::sample_type type, uint32_t device_id) const override;

            virtual rs::file_format::status read_next_sample(std::shared_ptr<ros_data_objects::sample>& sample) override;
            virtual rs::file_format::status get_file_duration(file_types::nanoseconds& duration) const override;
        ...
        private:
            ...
            rs::file_format::status seek_to_time(file_types::nanoseconds seek_time, std::unique_ptr<rosbag::View>& samples_view) const;

...
#include "include/msgs/sensor_msgs/Image.h"
...
#include "include/msgs/std_msgs/UInt32.h"
...
bool stream_playback::create(const std::string& file_path, std::unique_ptr<stream_playback>& player)
{
    try
    {
		player = std::unique_ptr<stream_playback_impl>(new stream_playback_impl(file_path));
    }
    catch (std::exception&)
    {
        return false;
...
bool stream_playback_impl::get_file_version_from_file(uint32_t& version)
{
...
stream_playback_impl::stream_playback_impl(const std::string &file_path)
{
    if (file_path.empty())
    {
...
stream_playback_impl::~stream_playback_impl()
{
}

status stream_playback_impl::set_filter(std::vector<std::string> topics)
{
...
status stream_playback_impl::seek_to_time(file_types::nanoseconds seek_time, std::unique_ptr<rosbag::View>& samples_view) const
{
...
status stream_playback_impl::seek_to_time(file_types::nanoseconds seek_time)
{
    std::unique_lock<std::mutex> locker(m_mutex);
...
status stream_playback_impl::read_vendor_data(std::vector<std::shared_ptr<ros_data_objects::vendor_data>>& vendor_data,
...
status stream_playback_impl::read_stream_infos(std::vector<std::shared_ptr<ros_data_objects::stream_info>>& stream_infos,
...
status stream_playback_impl::read_next_sample(std::shared_ptr<ros_data_objects::sample>& sample)
{
    std::unique_lock<std::mutex> locker(m_mutex);
...
std::shared_ptr<ros_data_objects::compressed_image> stream_playback_impl::create_compressed_image(const rosbag::MessageInstance &image_data) const
{
    ...
    return ros_data_objects::compressed_image::create(info);
}

std::shared_ptr<ros_data_objects::image> stream_playback_impl::create_image(const rosbag::MessageInstance &image_data) const
{
    ...
    return ros_data_objects::image::create(info);
}

std::shared_ptr<ros_data_objects::image_stream_info> stream_playback_impl::create_image_stream_info(const rosbag::MessageInstance &info_msg) const
{
    ...
    return ros_data_objects::image_stream_info::create(info);
}

std::shared_ptr<ros_data_objects::log> stream_playback_impl::create_log(const rosbag::MessageInstance &message) const
{
    ...
    info.line =  msg->line;
    return ros_data_objects::log::create(info);
}

std::shared_ptr<ros_data_objects::motion_sample> stream_playback_impl::create_motion_sample(const rosbag::MessageInstance &message) const
{
    ...
    return ros_data_objects::motion_sample::create(info);
}

std::shared_ptr<ros_data_objects::motion_stream_info> stream_playback_impl::create_motion_info(const rosbag::MessageInstance &message) const
{
    ...
    return ros_data_objects::motion_stream_info::create(info);
}

std::shared_ptr<ros_data_objects::occupancy_map> stream_playback_impl::create_occupancy_map(const rosbag::MessageInstance &message) const
{
    ...
    info.device_id = static_cast<uint32_t>(std::stoll(device_str));
    info.capture_time = nanoseconds(message.getTime().toNSec());
    return ros_data_objects::occupancy_map::create(info);
}

std::shared_ptr<ros_data_objects::property> stream_playback_impl::create_property(const rosbag::MessageInstance &message) const
{
    ...
    info.device_id = static_cast<uint32_t>(std::stoll(device_str));
    info.capture_time = nanoseconds(message.getTime().toNSec());
    return ros_data_objects::property::create(info);
}

std::shared_ptr<ros_data_objects::pose> stream_playback_impl::create_six_dof(const rosbag::MessageInstance &message) const
{
    ...
    info.device_id = static_cast<uint32_t>(std::stoll(device_str));
    info.capture_time = nanoseconds(message.getTime().toNSec());
    return ros_data_objects::pose::create(info);
}

std::shared_ptr<ros_data_objects::time_sample> stream_playback_impl::create_time_sample(const rosbag::MessageInstance &message) const
{
    ...
    return ros_data_objects::time_sample::create(info);
}

std::shared_ptr<ros_data_objects::vendor_data> stream_playback_impl::create_vendor_data(const rosbag::MessageInstance &message) const
{
    ...
    return ros_data_objects::vendor_data::create(info);
}
status stream_playback_impl::get_file_duration(file_types::nanoseconds& duration) const
{
    std::unique_lock<std::mutex> locker(m_mutex);
    std::unique_ptr<rosbag::View> samples_view;

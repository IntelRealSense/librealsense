...
#pragma omp parallel for schedule(dynamic)
    ...
    bool align::should_process(const rs2::frame& frame)
    {
        ...
        set.foreach([this, &has_tex](rs2::frame& frame) { if (frame.get_profile().stream_type() == _to_stream_type) has_tex = true; });
        set.foreach([&has_depth](rs2::frame& frame) 
    ...
    rs2::frame align::process_frame(const rs2::frame_source& source, rs2::frame f)
    {
        rs2::frame rv;
        ...
        if(_to_stream_type == RS2_STREAM_DEPTH)
            frames.foreach([&other_frames](rs2::frame& f) {if (f.get_profile().stream_type() != RS2_STREAM_DEPTH) other_frames.push_back(f); });
        else
            frames.foreach([this, &other_frames](rs2::frame& f) {if (f.get_profile().stream_type() == _to_stream_type) other_frames.push_back(f); });

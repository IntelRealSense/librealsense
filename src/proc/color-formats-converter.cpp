...
#undef clamp
    ...
    // It is expected that all branching outside of the loop control variable will be removed due to constant-folding.
    template<rs2_format FORMAT> void unpack_nv12(byte* const d[], const byte* s, int width, int height, int actual_size)
    {
        auto n = width * height;
        assert(n % 16 == 0); // All currently supported color resolutions are multiples of 16 pixels. Could easily extend support to other resolutions by copying final n<16 pixels into a zero-padded buffer and recursively calling self for final iteration.
#ifdef RS2_USE_CUDA
...
#pragma omp parallel for
            if (0)
            {
                for (int i = 0; i < n / 16; i++)
                {
                    ...
                    // Load 8 NV12 pixels each into two 16-byte registers
                    __m128i s0 = _mm_loadu_si128(&src[i * 2]);
                    __m128i s1 = _mm_loadu_si128(&src[i * 2 + 1]);
...
//#else  // Generic code for when SSSE3 is not available.
        auto src = reinterpret_cast<const uint8_t*>(s);
        auto dst = reinterpret_cast<uint8_t*>(d[0]);
        LOG_DEBUG("unpack_nv12 to format: " << rs2_format_to_string(FORMAT));
        for (; n; n -= 16, src += 24)
        {
            if (FORMAT == RS2_FORMAT_Y8)
            {
            ...
            if (FORMAT == RS2_FORMAT_Y16)
            {
                // Y16 is little-endian.  We output Y << 8.
                uint8_t out[32] = {
                    0, src[0], 0, src[1], 0, src[2], 0, src[3],
                    0, src[4], 0, src[5], 0, src[6], 0, src[7],
                    0, src[8], 0, src[9], 0, src[10], 0, src[11],
                    0, src[12], 0, src[13], 0, src[14], 0, src[15],
                };
                librealsense::copy(dst, out, sizeof out);
                dst += sizeof out;
                continue;
            ...
            int16_t y[16] = {
                ...
                src[16], src[16], src[16], src[16],
                src[18], src[18], src[18], src[18],
                src[20], src[20], src[20], src[20],
                src[22], src[22], src[22], src[22],
            }, v[16] = {
                src[17], src[17], src[17], src[17],
                src[19], src[19], src[19], src[19],
                src[21], src[21], src[21], src[21],
                src[23], src[23], src[23], src[23],
            };
...
#undef clamp
    ...
    void unpack_nv12(rs2_format dst_format, rs2_stream dst_stream, byte* const d[], const byte* s, int w, int h, int actual_size)
    {
        LOG_DEBUG("unpack nv12 called with dst_format: " << rs2_format_to_string(dst_format));
        switch (dst_format)
        {
        case RS2_FORMAT_Y8:
...
#undef clamp
    ...
    void nv12_converter::process_function(byte* const dest[], const byte* source, int width, int height, int actual_size, int input_size)
    {

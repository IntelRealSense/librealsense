...
enum class command_modifier : uint32_t
{
    direct = 0x10
...
namespace rsimpl
{
    namespace r200
    {
        ...
        void xu_read(const uvc::device & device, control xu_ctrl, void * buffer, uint32_t length) { uvc::get_control_with_retry(device, lr_xu, static_cast<int>(xu_ctrl), buffer, length); }

        void xu_write(uvc::device & device, control xu_ctrl, void * buffer, uint32_t length) { uvc::set_control_with_retry(device, lr_xu, static_cast<int>(xu_ctrl), buffer, length); }
        ...
        bool read_device_pages(uvc::device & dev, uint32_t address, unsigned char * buffer, uint32_t nPages)
        {
            ...
            if(!nPages || addressTest < 0) return false;
        ...
        void read_arbitrary_chunk(uvc::device & dev, uint32_t address, void * dataIn, int lengthInBytesIn)
        {
            ...
            if(startAddress & 0xff)
            {
                ...
                if(lengthToCopy > (uint32_t)lengthInBytes) lengthToCopy = lengthInBytes;
            ...
            if(nPagesToRead > 0) read_device_pages(dev, startAddress, data, nPagesToRead);
        ...
        void read_camera_head_contents(const uint8_t (&flash_data_buffer)[SPI_FLASH_SECTOR_SIZE_IN_BYTES], uint32_t & serial_number)
        {
            ...
            LOG_INFO("Module version                      = " << (int)header.moduleVersion << "." << (int)header.moduleMajorVersion << "." << (int)header.moduleMinorVersion << "."
                                                              << (int)header.moduleSkew);
        ...
        void set_stream_intent(uvc::device & device, uint8_t & intent) { xu_write(device, control::stream_intent, intent); }
        ...
        void set_emitter_state(uvc::device & device, bool state) { xu_write(device, control::emitter, uint8_t(state ? 1 : 0)); }

        void get_register_value(uvc::device & device, uint32_t reg, uint32_t & value) { value = send_command_and_receive_response(device, CommandResponsePacket(command::peek, reg)).value; }

        void set_register_value(uvc::device & device, uint32_t reg, uint32_t value) { send_command_and_receive_response(device, CommandResponsePacket(command::poke, reg, value)); }

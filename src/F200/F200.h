...
namespace f200
{
    ...
    struct CalibrationUtils
    {
        ...
        CalibrationUtils(uint32_t w, uint32_t h, rs_intrinsics rect, rs_intrinsics unrect, rs_extrinsics r)
        {
            width = w;
            height = h;
            ...
            RectificationTable(rect, r.rotation, unrect);
        }
        
        uint16_t * GetUndistortedImage(uint16_t * srcImg)
        {
        ...
        void RectificationTable(const rs_intrinsics & sourceIntrinsics, const float rotation[9], const rs_intrinsics & destIntrinsics)
        {
    ...
    class F200Camera : public rs::UVCCamera
    {
        ...
        std::unique_ptr<CalibrationUtils> calibUtils;

...
def usage():
    ...
    print(
        '        -t, --tag      configure all tests with the following tag. If used multiple times runs all tests matching')
...
def find_includes(filepath, filelist=set()):
    ...
    try:
        ...
        for include_line in file.grep(r'^\s*#\s*include\s+("(.*)"|<(.*)>)\s*$', filepath):
            ...
            include = find_include(m.group(2), filedir) or find_include_in_dirs(m.group(2)) or find_include_in_dirs(
                m.group(3))
            if include:
        ...
        try:
            ...
            for cmake_directive in file.grep('^//#cmake:\s*', dir + '/' + f):
                ...
                if cmd == 'add-file':
                    ...
                    log.e(f + '+' + str(
                        index) + ': unknown cmd \'' + cmd + '\' (should be \'add-file\', \'static!\', or \'shared!\')')
...
if len(shared_tests):
    ...
    handle.write('    message( INFO "' + str(
        len(shared_tests)) + ' shared lib unit-tests will be skipped. Check BUILD_SHARED_LIBS to run them..." )\n')
    handle.write('else()\n')
...
if len(static_tests):
    ...
    handle.write('    message( INFO "' + str(
        len(static_tests)) + ' static lib unit-tests will be skipped. Uncheck BUILD_SHARED_LIBS to run them..." )\n')
    handle.write('else()\n')

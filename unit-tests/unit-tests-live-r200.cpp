...
TEST_CASE("R200 infrared2 streaming modes exactly match infrared streaming modes", "[live] [r200]")
{
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        for (int j = 0; j < infrared_mode_count; ++j) {
            ...
            int infrared_width = 0, infrared_height = 0, infrared_framerate = 0;
            rs_format infrared_format = RS_FORMAT_ANY;
            ...
            int infrared2_width = 0, infrared2_height = 0, infrared2_framerate = 0;
            rs_format infrared2_format = RS_FORMAT_ANY;
            ...
            for (int k = 0; k < 5; ++k)
                REQUIRE(infrared_intrin.coeffs[k] == infrared_intrin.coeffs[k]);
        }
    }
}
...
enum { BEFORE_START_DEVICE = 1,
    AFTER_START_DEVICE = 2 };
...
TEST_CASE("a single R200 can stream a variety of reasonable streaming mode combinations", "[live] [r200] [one-camera]")
{
    ...
    {
        test_streaming(dev, { { RS_STREAM_DEPTH, 480, 360, RS_FORMAT_Z16, 60 } });
        ...
        test_streaming(dev, { { RS_STREAM_INFRARED, 492, 372, RS_FORMAT_Y16, 60 },
                                { RS_STREAM_INFRARED2, 492, 372, RS_FORMAT_Y16, 60 } });

...
def usage():
    ...
    print(
        '        -t, --tag        Run all tests with the following tag. If used multiple times runs all tests matching')
...
def check_log_for_fails(path_to_log, testname, configuration=None, repetition=1):
    ...
    if passed < total:
        ...
        if log.is_verbose_on():
            log.e(
                log.red + testname + log.reset + ': ' + configuration_str(configuration, repetition, suffix=' ') + desc)
            log.i('Log: >>>')
            log.out()
...
def test_wrapper(test, configuration=None, repetition=1):
    ...
    try:
        ...
        log.e(log.red + test.name + log.reset + ':',
              configuration_str(configuration, repetition, suffix=' ') + 'timed out')

...
TEST_CASE("Exposure - each sensor and both - streaming and display", "[F450]") {
    ...
    
    {
        ...
        check_option_from_max_to_min_and_back_streaming(sensor, RS2_OPTION_EXPOSURE, number_of_iterations, tolerance);
        //--------------------------------------------------------------------------
        check_option_from_max_to_min_and_back_streaming(sensor, RS2_OPTION_EXPOSURE_SECOND, number_of_iterations, tolerance);
        //--------------------------------------------------------------------------
        check_both_sensors_option_from_max_to_min_and_back_streaming(sensor, RS2_OPTION_EXPOSURE, RS2_OPTION_EXPOSURE_SECOND, number_of_iterations, tolerance);
    }
...
void check_option_from_max_to_min_and_back_streaming(const rs2::sensor& sensor, rs2_option opt, int number_of_iterations, float tolerance)
{
    ...
    checkOption_streaming2(sensor, opt, valueAfterChange, 15.0f,//range.min
...
void check_both_sensors_option_from_max_to_min_and_back_streaming(const rs2::sensor& sensor, rs2_option opt, rs2_option opt_second, int number_of_iterations, float tolerance)
{
    ...
    //going down dividing by 2 at each iteration until min
    checkOptionForBothSensors_streaming2(sensor, opt, opt_second, valueAfterChange, 15.0f, //range.min
     number_of_iterations, tolerance, [](float value, float limit) {
        return value > limit;
        },
        [&](float& value) {
    ...
    //then back to max multiplying by 2 until max
    checkOptionForBothSensors_streaming2(sensor, opt, opt_second, valueAfterChange, range.max, number_of_iterations, tolerance, [](float value, float limit) {
        return value < limit;
        },
        [&](float& value) {
            value *= 2.f;

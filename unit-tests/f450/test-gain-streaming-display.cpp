...
TEST_CASE("Gain - each sensor and both - streaming and display", "[F450]") {
    ...
    std::cout << "pipe started" << std::endl;
    {
        ...
        check_option_from_max_to_min_and_back_streaming(app, printer, color_map, pipe, sensor, RS2_OPTION_GAIN, number_of_iterations, tolerance);
        //--------------------------------------------------------------------------
        check_option_from_max_to_min_and_back_streaming(app, printer, color_map, pipe, sensor, RS2_OPTION_GAIN_SECOND, number_of_iterations, tolerance);
        //--------------------------------------------------------------------------
        check_both_sensors_option_from_max_to_min_and_back_streaming(app, printer, color_map, pipe, sensor, RS2_OPTION_GAIN, RS2_OPTION_GAIN_SECOND, number_of_iterations, tolerance);
    }
    pipe.stop(); 
    ...
    const rs2::pipeline& pipe, const rs2::sensor& sensor, rs2_option opt, int number_of_iterations, float tolerance)
{
    ...
    checkOption_streaming(app, printer, color_map, pipe, sensor, opt, valueAfterChange, range.min,
    ...
    const rs2::pipeline& pipe, const rs2::sensor& sensor, rs2_option opt, rs2_option opt_second, int number_of_iterations, float tolerance)
{
    ...
    //going down dividing by 2 at each iteration until min
    checkOptionForBothSensors_streaming(app, printer, color_map, pipe, sensor, opt, opt_second, valueAfterChange, range.min,
     number_of_iterations, tolerance, [](float value, float limit) {
        return value > limit;
        },
        [&](float& value) {
    ...
    //then back to max multiplying by 2 until max
    checkOptionForBothSensors_streaming(app, printer, color_map, pipe, sensor, opt, opt_second, valueAfterChange, range.max, 
        number_of_iterations, tolerance, [](float value, float limit) {
        return value < limit;
        },
        [&](float& value) {

...
TEST_CASE("Exposure - one value - while streaming", "[F450]") {
    using namespace std;
    ...
    std::cout << "pipe started" << std::endl;
    {
        ...
        float tolerance = 3; // for 3% - not absolute as done in gain
        ...
        bool result = (valueAfterChange >= (1.f - tolerance) * valueToSet) && 
            (valueAfterChange <= (1.f + tolerance) * valueToSet);
        REQUIRE(result);
        std::cout << std::endl;
        std::cout << "One Value Checked" << std::endl;
    }
    pipe.stop(); 

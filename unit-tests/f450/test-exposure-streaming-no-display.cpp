...
void check_option_from_max_to_min_and_back_streaming(rs2::pipeline pipe, rs2::sensor sensor, rs2_option opt, int number_of_iterations, float tolerance);
void check_both_sensors_option_from_max_to_min_and_back_streaming(rs2::pipeline pipe, rs2::sensor sensor, rs2_option opt, rs2_option opt_second, int number_of_iterations, float tolerance);
...
TEST_CASE("Exposure - each sensor and both - no streaming", "[F450]") {
    ...
    std::cout << "pipe started" << std::endl;
    {
        ...
        check_option_from_max_to_min_and_back_streaming(pipe, sensor, RS2_OPTION_EXPOSURE, number_of_iterations, tolerance);
        check_option_from_max_to_min_and_back_streaming(pipe, sensor, RS2_OPTION_EXPOSURE_SECOND, number_of_iterations, tolerance);
        check_both_sensors_option_from_max_to_min_and_back_streaming(pipe, sensor, RS2_OPTION_EXPOSURE, RS2_OPTION_EXPOSURE_SECOND, number_of_iterations, tolerance);
...
void check_option_from_max_to_min_and_back_streaming(rs2::pipeline pipe, rs2::sensor sensor, rs2_option opt, int number_of_iterations, float tolerance)
{
    ...
    checkOption_streaming(pipe, sensor, opt, valueAfterChange, 15.0f/*range.min*/, number_of_iterations, tolerance, [](float value, float limit) {
        return value > limit;
        },
        [&](float& value) {
            value /= 2.f;
    ...
    checkOption_streaming(pipe, sensor, opt, valueAfterChange, range.max, number_of_iterations, tolerance, [](float value, float limit) {
        return value < limit;
        },
        [&](float& value) {
            value *= 2.f;
...
void check_both_sensors_option_from_max_to_min_and_back_streaming(rs2::pipeline pipe, rs2::sensor sensor, rs2_option opt, rs2_option opt_second, int number_of_iterations, float tolerance)
{
    ...
    //going down dividing by 2 at each iteration until min
    checkOptionForBothSensors_streaming(pipe, sensor, opt, opt_second, valueAfterChange, 15.0f/*range.min*/, number_of_iterations, tolerance, [](float value, float limit) {
        return value > limit;
        },
        [&](float& value) {
            value /= 2.f;
    ...
    //then back to max multiplying by 2 until max
    checkOptionForBothSensors_streaming(pipe, sensor, opt, opt_second, valueAfterChange, range.max, number_of_iterations, tolerance, [](float value, float limit) {
        return value < limit;
        },
        [&](float& value) {
            value *= 2.f;

...
try:
    bridge.open( servers['Gyro'].default_profile() )  # MXYZ @ 200 Hz
    with bridge_error_expected( "failure trying to start/stop 'Accel': profile <'Accel' MXYZ @ 100 Hz> is incompatible with already-open <'Gyro' MXYZ @ 200 Hz>" ):
        start_stream( 'Accel' )

...
def usage():
    ...
    print( 'Syntax: ' + ourname + ' <dir> <regex>' )
    print( '        Both parameters are optional')
    print( '        dir: the directory holding the executable test files to run. If not specified we try to assume')
    print( '        one ourselves from build directory. This fails if there are 2 versions of tests, example:')
    print( '        tests built in Debug and test built in Release')
    print( '        regex: run all tests that fit <regex>. If not specified, run all tests')
    print( 'Options:' )
...
if not target:
    ...
    for executable in find(build, '(^|/)test-.*'):
        ...
        if target and target != dir_with_test:
            error("Two versions of tests found!")
            usage()

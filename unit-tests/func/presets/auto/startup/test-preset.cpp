...
TEST_CASE( "calc preset from controls on start up", "[l500][live]" )
{
    ...
    build_new_device_and_do( [&]( rs2::depth_sensor & depth_sens ) {
        ...
        REQUIRE_NOTHROW( depth_sens.set_option( RS2_OPTION_VISUAL_PRESET,
                                                (float)RS2_L500_VISUAL_PRESET_AUTOMATIC ) );
    } );
    ...
    build_new_device_and_do( [&]( rs2::depth_sensor & depth_sens ) {
        CAPTURE( rs2_l500_visual_preset( RS2_L500_VISUAL_PRESET_AUTOMATIC ) );
        check_preset_is_equal_to( depth_sens,
                                  rs2_l500_visual_preset( RS2_L500_VISUAL_PRESET_AUTOMATIC ) );
    } );

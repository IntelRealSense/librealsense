...
TEST_CASE( "check defaults after resolution changed", "[l500][live]" )
{
    ...
    auto preset_to_expected_map = build_preset_to_expected_defaults_map( dev, depth_sens );

    // print_presets_to_csv( depth_sens, preset_to_expected_map );
    for_each_preset_mode_combination(
        [&]( rs2_l500_visual_preset preset, rs2_sensor_mode from_mode ) {
            for( int to_mode = RS2_SENSOR_MODE_VGA; to_mode < RS2_SENSOR_MODE_COUNT; to_mode++ )
            {
                CAPTURE( preset, from_mode, rs2_sensor_mode(to_mode) );
                ...
                auto & preset_to_expected
                    = preset_to_expected_map[{ rs2_l500_visual_preset( preset ),
                                               rs2_sensor_mode( to_mode ) }];

                compare_expected_defaults_to_actual( depth_sens, preset_to_expected );
            }
        } );
}
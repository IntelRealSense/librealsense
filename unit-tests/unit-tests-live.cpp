...
TEST_CASE("synthetic streaming mode properties are correct", "[live]")
{
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        for (int j = 0; j < color_mode_count; ++j) {
            ...
            int color_width = 0, color_height = 0, color_framerate = 0;
            rs_format color_format = RS_FORMAT_ANY;
            ...
            for (int k = 0; k < 5; ++k)
                REQUIRE(rectified_color_intrin.coeffs[k] == 0.0f);

            for (int k = 0; k < depth_mode_count; ++k) {
                // Enable a DEPTH mode and retrieve intrinsics
                int depth_width = 0, depth_height = 0, depth_framerate = 0;
                rs_format depth_format = RS_FORMAT_ANY;
                ...
                for (int l = 0; l < 5; ++l)
                    REQUIRE(color_aligned_to_depth_intrin.coeffs[l] == depth_intrin.coeffs[l]);
                ...
                for (int l = 0; l < 5; ++l)
                    REQUIRE(depth_aligned_to_color_intrin.coeffs[l] == color_intrin.coeffs[l]);
                ...
                for (int l = 0; l < 5; ++l)
                    REQUIRE(depth_aligned_to_rectified_color_intrin.coeffs[l] == rectified_color_intrin.coeffs[l]);
            }
        }
    }
}

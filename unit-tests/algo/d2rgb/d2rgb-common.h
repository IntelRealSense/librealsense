...
void init_algo( algo::optimizer & cal,
    ...
    std::string const & yuy_valid_prev,
    std::string const & ir,
    std::string const & z,
...
)
{
    ...
    std::vector< algo::yuy_t> yuy_valid_prev_frame;

    try
    {
        yuy_valid_prev_frame = read_image_file< algo::yuy_t >(dir + yuy_valid_prev, camera.rgb.width, camera.rgb.height);
    }
    catch (...) 
    {
        yuy_valid_prev_frame.resize(camera.rgb.width*camera.rgb.height, 0);
    };
